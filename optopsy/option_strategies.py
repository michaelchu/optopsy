...
def long_call(data, kwargs):
    return OrderAction.BTO, single(
        data, OptionType.CALL, _merge_params(kwargs))


def short_call(data, kwargs):
    return OrderAction.STO, single(
        data, OptionType.CALL, _merge_params(kwargs))
...
def long_call_spread(data, kwargs):
    return OrderAction.BTO, vertical(
        data, OptionType.CALL, _merge_params(kwargs))


# credit
def short_call_spread(data, kwargs):
    return OrderAction.STO, vertical(
        data, OptionType.CALL, _merge_params(kwargs))
...
def long_put_spread(data, kwargs):
    return OrderAction.BTO, vertical(
        data, OptionType.PUT, _merge_params(kwargs))
