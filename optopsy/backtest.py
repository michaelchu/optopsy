...
sort_by = ['underlying_symbol', 'quote_date', 'option_type', 'expiration', 'strike']
...
def _filter_data(data, filters):
    ...
    return pd.concat(_apply_filters(data, entry_filters)).sort_values(sort_by)
...
def run(data, trades, filters):

    trades = trades if isinstance(trades, list) else [trades]
    
    #merge trades from multiple underlyings if applicable
    ...
    results = pd.merge(all_trades, data, on=on).pipe(_filter_data, exit_filters)
